from cardClass import Government, Production, Urban, Military, Action, Technology, Leader, Wonder, Event, Pact, Colony, Impact, Aggression, War, Bonus, Tactic
...
class Decks:
    ...
    Conmon = Government('Constitutional Monarchy',
                        'II', 1, 2, 2, 6, 4, 3, 12, 6)
    ...
    Fundamentalism = Government(
        'Fundamentalism', 'III', 1, 1, 1, 6, 5, 4, 16, 5)
    ...
    Selective_Breeding = Production(
        'Selective_Breeding', 'II', 1, 2, 3, 'farm', 3, 5, 6)

    # Age III Production Cards
    Oil = Production('Oil', 'III', 1, 2, 2, 'mine', 5, 9, 11)
    Mechanized_Agriculture = Production(
        'Mechanized Agriculture', 'III', 1, 2, 2, 'farm', 5, 7, 8)
    ...
    Printing_Press = Urban('Printing Press', 'I', 2, 2,
                           2, 'library', 1, 1, 0, 0, 3, 3)
    Bread_and_Circuses = Urban(
        'Bread and Circuses', 'I', 1, 2, 2, 'arena', 0, 0, 1, 2, 3, 3)
    Drama = Urban('Drama', 'I', 1, 2, 2, 'theatre', 0, 2, 0, 1, 3, 4)

    # Age II Urban Buildings
    Organized_Religion = Urban(
        'Organized Religion', 'II', 2, 2, 2, 'temple', 0, 1, 0, 3, 4, 7)
    Scientific_Method = Urban(
        'Scientific Method', 'II', 2, 2, 2, 'lab', 3, 0, 0, 0, 6, 8)
    Journalism = Urban('Journalism', 'II', 1, 2, 2,
                       'library', 2, 2, 0, 0, 6, 8)
    Team_Sports = Urban('Team Sports', 'II', 1, 1,
                        1, 'arena', 0, 0, 2, 3, 5, 5)
    ...
    Multimedia = Urban('Multimedia', 'III', 2, 2, 2,
                       'library', 3, 3, 0, 0, 9, 11)
    Professional_Sports = Urban(
        'Professional Sports', 'III', 1, 2, 2, 'arena', 0, 0, 4, 4, 7, 7)
    ...
    # Age III Military
    Modern_Infantry = Military(
        'Modern Infantry', 'III', 1, 2, 2, 'infantry', 5, 10, 7)
    ...
    # Age I Technologies
    Cartography = Technology('Cartography', 'I', 1, 1,
                             1, 'exploration', 0, 0, 1, 2, 0, 0, 4)
    Warfare = Technology('Warfare', 'I', 1, 2, 2,
                         'warfare', 0, 1, 1, 0, 0, 0, 5)
    Code_of_Laws = Technology('Code of Laws', 'I', 1,
                              2, 2, 'law', 1, 0, 0, 0, 0, 0, 6)
    Masonry = Technology('Masonry', 'I', 1, 1, 1,
                         'construction', 0, 0, 0, 0, 2, [1, 1, 1], 3)

    # Age II Technologies
    Navigation = Technology('Navigation', 'II', 1, 1, 1,
                            'exploration', 0, 0, 2, 3, 0, 0, 6)
    Strategy = Technology('Strategy', 'II', 1, 1, 1,
                          'warfare', 0, 2, 3, 0, 0, 0, 8)
    Justice_System = Technology(
        'Justice System', 'II', 1, 1, 1, 'law', 1, 0, 0, 0, 0, 0, 7)
    Justice_System.blue_cubes = 3
    Architecture = Technology('Architecture', 'II', 1,
                              2, 2, 'construction', 0, 0, 0, 0, 3, [1, 2, 2], 6)

    # Age III Technologies
    Satellites = Technology('Satellites', 'III', 1, 1,
                            1, 'exploration', 0, 0, 3, 4, 0, 0, 8)
    Military_Theory = Technology(
        'Military Theory', 'III', 1, 2, 2, 'warfare', 0, 3, 5, 0, 0, 0, 11)
    Civil_Service = Technology(
        'Civil Service', 'III', 1, 1, 1, 'law', 2, 0, 0, 0, 0, 0, 10)
    Civil_Service.blue_cubes = 3
    Engineering = Technology('Engineering', 'III', 1,
                             1, 1, 'construction', 0, 0, 0, 0, 4, [1, 2, 3], 9)
    ...
    Library_of_Alexandria = Wonder(
        'Library of Alexandria', 'A', 0, 0, 0, False)
    ...
    Knowledge_of_the_Ancients = Event(
        'Knowledge of the Ancients', 'I', 1, 1, 1)
    ...
    Popularization_of_Science = Event(
        'Popularization of Science', 'II', 1, 1, 1)
    ...
    International_Negotiations = Event(
        'International Negotiations', 'II', 1, 1, 1)
    ...
    Armed_Intervention = Aggression(
        'Aggression: Armed Intervention', 'III', 4, 4, 4)
    ...
    def build_civil_deck(self):
        ...
        self.full_civil_deck = self.age_A_civil_deck + self.age_I_civil_deck + \
            self.age_II_civil_deck + self.age_III_civil_deck
    ...
    def build_military_deck(self):
        ...
        self.full_military_deck = self.age_A_military_deck + self.age_I_military_deck + \
            self.age_II_military_deck + self.age_III_military_deck
